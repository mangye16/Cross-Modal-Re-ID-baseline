...
from utils import *
from loss import OriTripletLoss, TripletLoss_WRT, KLDivLoss, TripletLoss_ADP,CoRefineLoss
...
parser.add_argument('--S', default= 0 , type=int,
                    metavar='S', help='strong for label')
parser.add_argument('--S2', default= 0 , type=int,
                    metavar='S2', help='strong for label')
...
if dataset == 'sysu':
    data_path = '/omnisky3/public-datasets/ccq/SYSU-MM01/'
    log_path = args.log_path + 'sysu_log/'
    test_mode = [1, 2]  # thermal to visible
elif dataset == 'regdb':
    data_path = '/omnisky3/public-datasets/ccq/RegDB/'
...
start_epoch = 0
best_mINP = 0  # best test accuracy
best_map = 0  # best test accuracy
...
loss_ws = CoRefineLoss().to(device)
...
def train(epoch):
    ...
    for batch_idx, (input10, input11, input2, label1, label2) in enumerate(trainloader):
        ...
        out2 = out0.narrow(0,n,n)
        out3 = out0.narrow(0,2*n,n)

        if args.S == 1:
            loss_kl = (loss_ws(out1, out2.detach()) + loss_ws(out3, out2.detach()))

        elif args.S2 == 1:
            loss_kl = loss_ws(out2, out1.detach())
        else:
            loss_kl = loss_ws(out1, out2.detach())
...
for epoch in range(start_epoch, 100 - start_epoch):
    ...
    if epoch > 0 and epoch % 2 == 0:
        ...
        if mINP_att > best_mINP:  # not the real best for sysu-mm01
            best_mINP = mINP_att
            best_epoch = epoch
            state = {
                'net': net.state_dict(),
            ...
            torch.save(state, checkpoint_path + suffix + '_mINP_best.t')
